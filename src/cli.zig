const std = @import("std");

pub const DEFAULT_SYSTEM_PROMPT = "You are a helpful assistant specialized in software development tasks.";
pub const DEFAULT_TEMPERATURE: f32 = 0.7;

pub const CliArgs = struct {
    model_path: []const u8 = "",
    system_prompt: []const u8 = DEFAULT_SYSTEM_PROMPT,
    temperature: f32 = DEFAULT_TEMPERATURE,
    show_help: bool = false,
    // Track which strings were allocated so we can free them
    model_path_allocated: bool = false,
    system_prompt_allocated: bool = false,
};

pub const ParseError = error{
    MissingModelPath,
    MissingSystemPrompt,
    MissingTemperature,
    InvalidFloat,
    OutOfMemory,
};

pub fn parseArgs(allocator: std.mem.Allocator) ParseError!CliArgs {
    var args_iter = std.process.argsWithAllocator(allocator) catch return error.OutOfMemory;
    defer args_iter.deinit();

    _ = args_iter.next(); // Skip executable name

    var args = CliArgs{};

    while (args_iter.next()) |arg| {
        if (std.mem.eql(u8, arg, "-h") or std.mem.eql(u8, arg, "--help")) {
            args.show_help = true;
        } else if (std.mem.eql(u8, arg, "-m") or std.mem.eql(u8, arg, "--model")) {
            const value_arg = args_iter.next() orelse return error.MissingModelPath;
            args.model_path = allocator.dupe(u8, value_arg) catch return error.OutOfMemory;
            args.model_path_allocated = true;
        } else if (std.mem.eql(u8, arg, "-s") or std.mem.eql(u8, arg, "--system")) {
            const value_arg = args_iter.next() orelse return error.MissingSystemPrompt;
            args.system_prompt = allocator.dupe(u8, value_arg) catch return error.OutOfMemory;
            args.system_prompt_allocated = true;
        } else if (std.mem.eql(u8, arg, "-t") or std.mem.eql(u8, arg, "--temp")) {
            const temp_str = args_iter.next() orelse return error.MissingTemperature;
            args.temperature = std.fmt.parseFloat(f32, temp_str) catch return error.InvalidFloat;
        } else {
            std.debug.print("Unknown argument: {s}\n", .{arg});
            args.show_help = true;
            break;
        }
    }

    return args;
}

pub fn freeArgs(allocator: std.mem.Allocator, args: *CliArgs) void {
    if (args.model_path_allocated) {
        allocator.free(args.model_path);
    }
    if (args.system_prompt_allocated) {
        allocator.free(args.system_prompt);
    }
}

pub fn printHelp(exe_name: []const u8) void {
    std.debug.print(
        \\Usage: {s} [options]
        \\
        \\A simple CLI chat interface for Qwen3 models using llama.cpp.
        \\
        \\Options:
        \\  -m, --model <path>    Path to the GGUF model file (required)
        \\  -s, --system <prompt> System prompt to use (default: "{s}")
        \\  -t, --temp <value>    Sampling temperature (default: {d:.1})
        \\  -h, --help            Show this help message
        \\
    , .{ exe_name, DEFAULT_SYSTEM_PROMPT, DEFAULT_TEMPERATURE });
}
